...
def set_debug_log_only(debug_log_only):
    print("set_debug_log_only", debug_log_only)
    global _debug_log_only
    _debug_log_only = debug_log_only

def get_debug_log_only():
    return _debug_log_only
...
def log(s, with_prefix=True, with_timestamp=True, color=None):
    ...
    if not _log_tabular_only and not _debug_log_only:
        # Also log to stdout
        print(out)
...
def dump_tabular(*args, **kwargs):
    ...
    if len(_tabular) > 0:
        if _log_tabular_only and not _debug_log_only:
...
def save_itr_params(itr, params, torch_params=None):
    if _snapshot_dir:
        if _snapshot_mode == 'all':
            if not params is None:
                file_name = osp.join(_snapshot_dir, 'itr_%d.pkl' % itr)
                joblib.dump(params, file_name, compress=3)
            if not torch_params is None:
                for key, value in torch_params.items():
                    file_name = osp.join(_snapshot_dir, 'itr_%d_model.pkl' % itr)
            ...
            if not torch_params is None:
                for key, value in torch_params.items():
                    file_name = osp.join(_snapshot_dir, 'model.pkl' % itr)
                    torch.save(value.state_dict(), file_name)
