...
class NormalizedEnv(ProxyEnv, Serializable):
    def __init__(
        ...
        self._use_normalization_bounds = use_normalization_bounds
    ...
    def _apply_normalize_obs(self, obs):
        if self._use_normalization_bounds:
            if isinstance(self._wrapped_env.observation_space, Box):
                # rescale the action
                lb, ub = self._wrapped_env.observation_space.bounds
                normalized_observation = (2*(obs-lb)/(ub - lb))-1
                normalized_observation = np.clip(normalized_observation, lb, ub)
            else:
    ...
    @property
    def _state(self):
        return self._wrapped_env._state

    def _set_state(self, state):
        return self._wrapped_env._set_state(state)

    def action_from_keys(self, keys):
        return self._wrapped_env.action_from_keys(keys)
